...
-(void)WebServiceRequestCompleted:(NSDictionary *)result context:(NSString *)context
{
    ...
    NSArray *array1=[result objectForKey:@"ns1.JSONEmployeeServiceExceptionDetails"];
    
    NSDictionary *dict1=[array1 objectAtIndex:[[result objectForKey:@"faultMessage"]integerValue]];
    
    NSString *str1=[dict1 objectForKey:@"faultMessage"]
    ;

    if ([str1 isEqualToString:@"Attendance not found for this student"])
    {
        [Utility showToast:@"Attendance not found for this student" inView:self.view];
