...
//
#define keyboardHeightIncludingHeader 256
...
- (void)viewDidAppear:(BOOL)animated{
  ...
  [detailscroll setContentSize:CGSizeMake(self.view.frame.size.width, self.view.frame.size.height)];
  NSLog(@"%@",(NSStringFromCGRect(detailscroll.frame)));
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
  ...
  [UIView animateWithDuration:0.6 animations:^{
    ...
    viewHeight = viewHeight - keyboardHeightIncludingHeader;
    CGFloat btnActualFrame = searchimg.frame.origin.y + searchimg.frame.size.height-(64 + admissionno.frame.size.height);
    CGFloat yFinal = viewHeight - btnActualFrame;
    rect.origin.y = yFinal;
